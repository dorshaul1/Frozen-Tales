...
try{
 ...
 localStorage.removeItem('arctic-drift.check-cargo-fight');
 const scene=new RiverScene('arctic-drift.check-cargo-fight',true),game=new Phaser.Game({type:Phaser.AUTO,parent:'test',width:1000,height:680,pixelArt:true,physics:{default:'arcade'},scene:[scene]});
 ...
 for(const id of ['whitefish'] as FishId[]){const spot=scene.fishing.spots[0];Object.assign(spot,{x:832,y:1200,availableAt:0,retired:false,weights:{[id]:1}});body.reset(892,1200);await wait(100);while(!scene.cargo.full)scene.cargo.add('whitefish'); const before=scene.cargo.count;key(69,true);await wait(280);check(scene.fishing.state==='aiming','Hold charges directional cast');key(69,false);await wait(200);check(scene.fishing.active,'Release casts into assisted school');
 ...
 for(const code of [69,65,68,83,87])key(code,false);check(scene.cargo.count===before&&scene.harborPanel.isOpen,'Full hold still allows real cast/fight and opens decision'); scene.harborPanel.close();await wait(4800);}
