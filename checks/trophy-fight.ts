...
try{
 ...
 localStorage.removeItem('arctic-drift.check-trophy-fight');
 const scene=new RiverScene('arctic-drift.check-trophy-fight',true),game=new Phaser.Game({type:Phaser.AUTO,parent:'test',width:1000,height:680,pixelArt:true,physics:{default:'arcade'},scene:[scene]});
 ...
 for(const id of ['salmon'] as FishId[]){const [l,r]=banks(4630),cx=(l+r)/2;const spot=scene.fishing.spots[0];Object.assign(spot,{x:cx-50,y:4630,availableAt:0,retired:false,weights:{[id]:1}});body.reset(cx+10,4630);await wait(100);const before=scene.cargo.count;const previousRandom=Math.random;Math.random=()=>.01;key(69,true);await wait(280);check(scene.fishing.state==='aiming','Hold charges directional cast');key(69,false);await wait(200);check(scene.fishing.active,'Release casts into assisted school');
 ...
 for(const code of [69,65,68,83,87])key(code,false);check(scene.cargo.count===before+1, String(scene.fishing.fight?.outcome)+' '+scene.fishing.state+' '+Reflect.get(scene.fishing,'result')+' '+id+': cast → fight → tire → reel → land reaches existing inventory');check(scene.cargo.entries.at(-1)?.size==='trophy'&&scene.cargo.trophies.salmon===1,'The fought specimen is stored with its trophy status');Math.random=previousRandom;await wait(4800);}
 const cx=850; const preview=document.createElement('button');preview.textContent='Fight preview';preview.onclick=()=>{body.reset(892,1200);const target=scene.fishing.spots[0];Reflect.set(scene.fishing,'phase','hooked');Reflect.set(scene.fishing,'target',target);Reflect.set(scene.fishing,'hookedFish','crown');Reflect.set(scene.fishing,'castPoint',{x:cx-50,y:4630});Reflect.set(scene.fishing,'fishPoint',{x:cx-50,y:4630});Reflect.set(scene.fishing,'currentFight',new FishFight(scene.equipment.fightFor('crown')));scene.fishing.fight!.controlAngle=Math.PI;scene.fishing.update(0,false,false);scene.scene.pause();};document.body.prepend(preview);
