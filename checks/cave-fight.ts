...
try{
 ...
 localStorage.removeItem('arctic-drift.check-cave-fight');
 const scene=new RiverScene('arctic-drift.check-cave-fight',true),game=new Phaser.Game({type:Phaser.AUTO,parent:'test',width:1000,height:680,pixelArt:true,physics:{default:'arcade'},scene:[scene]});
 ...
 scene.equipment.save();const saved=new SaveStore('arctic-drift.check-cave-fight').load();
 check(['cisco','glasschar','veil'].every(id=>!!saved.records[id as FishId]),'New species records survive save/load');
