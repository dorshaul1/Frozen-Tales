...
try{
 ...
 localStorage.removeItem('arctic-drift.check-gorge-fight');
 const scene=new RiverScene('arctic-drift.check-gorge-fight',true),game=new Phaser.Game({type:Phaser.AUTO,parent:'test',width:1000,height:680,pixelArt:true,physics:{default:'arcade'},scene:[scene]});
 ...
 for(const id of ['dolly','lenok','sleeper'] as FishId[]){const [l,r]=banks(4630),cx=(l+r)/2;const spot=scene.fishing.spots[0];Object.assign(spot,{x:cx-50,y:4630,availableAt:0,retired:false,weights:{[id]:1}});body.reset(cx+10,4630);await wait(100);const before=scene.cargo.count;key(69,true);await wait(280);check(scene.fishing.state==='aiming','Hold charges directional cast');key(69,false);await wait(200);check(scene.fishing.active,'Release casts into assisted school');
