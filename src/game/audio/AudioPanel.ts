...
export class AudioPanel {
  ...
  constructor(private scene:Phaser.Scene,private audio:AudioManager,private onOpen:()=>void,private onClose:()=>void){
    ...
    this.panel.add(scene.add.zone(140,-132,24,24).setOrigin(0).setInteractive({useHandCursor:true}).on('pointerdown',()=>this.close()));
    PANEL_CHANNELS.forEach((id,i)=>{
      ...
      const slider=scene.add.zone(-65,y-15,130,27).setOrigin(0).setInteractive({useHandCursor:true});
      slider.on('pointerdown',(p:Phaser.Input.Pointer)=>{this.selection=i;this.dragging=id;this.slide(p);});this.panel.add(slider);
      this.panel.add(scene.add.zone(107,y-16,51,28).setOrigin(0).setInteractive({useHandCursor:true}).on('pointerdown',()=>{this.selection=i;audio.setMute(id,!audio.settings[id].muted);this.draw();}));
    });
    this.panel.add(text(-155,112,'↑↓ choose  ←→ volume  E mute  Esc close',8));
    this.helpButton=text(-155,88,'Controls & How to Play  >',10,'#f3d49a').setInteractive({useHandCursor:true}).on('pointerdown',()=>this.showHelp(0));
    ...
    scene.input.on('pointermove',this.slide,this);scene.input.on('pointerup',this.release,this);
    scene.game.events.on(Phaser.Core.Events.BLUR,this.close,this);
    scene.events.once('shutdown',()=>{this.button?.removeEventListener('click',this.toggle);this.button?.removeEventListener('keydown',this.buttonKey);scene.input.off('pointermove',this.slide,this);scene.input.off('pointerup',this.release,this);scene.game.events.off(Phaser.Core.Events.BLUR,this.close,this);this.panel.destroy();});
  }
  private showHelp(page:number) {
    ...
    this.helpView.add(this.scene.add.image(0,0,ATLAS,ASSETS['hub-panel']).setInteractive());
    this.helpView.add(text(-155,-125,data.title,'#f3d49a'));
    data.lines.forEach((line,i)=>this.helpView.add(text(-155,-90+i*14,line)));
    this.helpView.add(text(-155,102,'< '+(page+1)+' / 2  · arrows / E for next >','#f3d49a').setInteractive({useHandCursor:true}).on('pointerdown',()=>this.showHelp((page+1)%HELP_PAGES.length)));
    this.helpView.add(text(-155,123,'Esc · back to pause','#c0dce0').setInteractive({useHandCursor:true}).on('pointerdown',()=>this.showHelp(-1)));
  }
  ...
  private buttonKey=(event:KeyboardEvent)=>{if((event.code==='Enter'||event.code==='Space')&&!event.repeat){event.preventDefault();event.stopPropagation();this.toggle();}};
